# GraphUnzip - article branch

[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.4291093.svg)](https://doi.org/10.5281/zenodo.4291093)

Unzips an assembly graph using Hi-C data and/or long reads and/or linked reads. This branch is meant to reproduce the results of the article. Use the main branch if you just want to use GraphUnzip.

## Reproducibility

Here are detailed all the pipelines used to obtain the different assemblies of the paper. The change of directories are omitted, so be careful if you try to run these commands.

### Puccina triticina haploid assembly without GraphUnzip

```
#initial assembly
hifiasm -o hifiasm.asm data/HiFi/SRR14424683.1.fastq -t 8 -m 100000 -l 0

#purge_haplotigs
minimap2 -t 8 -ax map-hifi hifiasm.asm.bp.p_utg.fa data/HiFi/SRR14424683.1.fastq --secondary=no | sort reads_on_ref.sam -m 1G -o aligned.bam -T tmp.ali
purge_haplotigs hist -b aligned.bam -g hifiasm.asm.bp.p_utg.fa -t 4
purge_haplotigs cov -i aligned.bam.gencov -l 15 -m 80 -h 150
purge_haplotigs purge -g hifiasm.asm.bp.p_utg.fa -c coverage_stats.csv

#mapping Hi-C to curated assembly
hicstuff pipeline -D -t 4 -o hicstuff/ --genome curated.fasta -m iterative -e DpnII -n data/Hi-C/SRR14386306.1_1.fastq data/Hi-C/SRR14386306.1_2.fastq
samtools cat hicstuff/tmp/*.bam > hicstuff/tmp/hic.bam
samtools sort hicstuff/tmp/hic.bam > hicstuff/tmp/hic_sorted.bam
bamToBed -i hicstuff/tmp/hic_sorted.bam > hicstuff/tmp/hic_sorted.bed

#salsa
run_pipeline.py -a curated.fasta -l curated.fasta.fai -b hicstuff/tmp/hic_sorted.bed

#yahs
yahs curated.fasta hicstuff/tmp/hic_sorted.bam

```

### Puccina triticina haploid assembly with GraphUnzip
```
#initial assembly
hifiasm -o hifiasm.asm data/HiFi/SRR14424683.1.fastq -t 8 -m 100000 -l 0

#mapping Hi-C to raw assembly
hicstuff pipeline -D -t 4 -o hicstuff/ --genome hifiasm.asm.bp.p_utg.fa -m iterative -e DpnII -n data/Hi-C/SRR14386306.1_1.fastq data/Hi-C/SRR14386306.1_2.fastq

#graphunzip !
python graphunzip.py HiC-IM -g hifiasm.asm.bp.p_utg.gfa -m hicstuff/abs_fragments_contacts_weighted.txt -F fragments_list.txt -i interactionMatrix
python graphunzip.py unzip -r -g hifiasm.asm.bp.p_utg.gfa -i interactionMatrix -o unzipped_merged.gfa -f unzipped_merged.fasta --haploid

#purge_haplotigs
minimap2 -t 8 -ax map-hifi unzipped_merged.fasta data/HiFi/SRR14424683.1.fastq --secondary=no | sort reads_on_ref.sam -m 1G -o aligned.bam -T tmp.ali
purge_haplotigs hist -b aligned.bam -g hifiasm.asm.bp.p_utg.fa -t 4
purge_haplotigs cov -i aligned.bam.gencov -l 15 -m 80 -h 150
purge_haplotigs purge -g hifiasm.asm.bp.p_utg.fa -c coverage_stats.csv
...
yahs curated.fasta hicstuff/tmp/hic_sorted.bam
```
Note that mapping of the Hi-C reads was done twice. This is not necessary anymore with the -b option of graphunzip.

### Adineta vaga haploid assembly without GraphUnzip
```
#initial assembly
flye --nano-raw data/Nanopore/SRR13348928.fastq -o . -t 8

#purge_haplotigs
purge_haplotigs hist -b aligned.bam -g assembly_graph.fa
purge_haplotigs cov -i aligned.bam.gencov -l 25 -h 185 -m 85
purge_haplotigs purge -g ../assembly_graph.fa -c coverage_stats.csv

#mapping Hi-C to curated assembly
hicstuff pipeline -D -e DpnII -g curated.fa -m iterative -n -t 4 --force data/Hi-C/adineta_pcrfree.end1.fq  data/Hi-C/adineta_pcrfree.end2.fq
samtools cat hicstuff/tmp/*.bam > hicstuff/tmp/hic.bam
samtools sort hicstuff/tmp/hic.bam > hicstuff/tmp/hic_sorted.bam
bamToBed -i hicstuff/tmp/hic_sorted.bam > hicstuff/tmp/hic_sorted.bed
...
### Adineta vaga haploid assembly with GraphUnzip
```
#initial assembly
flye --nano-raw data/Nanopore/SRR13348928.fastq -o . -t 8

#mapping Hi-C to raw assembly
hicstuff pipeline -D -t 4 -o hicstuff/ --genome assembly_graph.fa -m iterative -e DpnII -n data/Hi-C/adineta_pcrfree.end1.fq  data/Hi-C/adineta_pcrfree.end2.fq

#graphunzip !
python graphunzip.py HiC-IM -g assembly_graph.gfa -m hicstuff/abs_fragments_contacts_weighted.txt -F fragments_list.txt -i interactionMatrix
python graphunzip.py unzip -r -g assembly_graph.gfa -i interactionMatrix -o unzipped_merged.gfa -f unzipped_merged.fasta --haploid

#purge_haplotigs
minimap2 -t 8 -ax map-hifi unzipped_merged.fasta data/Nanopore/SRR13348928.fastq --secondary=no | sort reads_on_ref.sam -m 1G -o aligned.bam -T tmp.ali
purge_haplotigs hist -b aligned.bam -g hifiasm.asm.bp.p_utg.fa -t 4
purge_haplotigs cov -i aligned.bam.gencov -l 15 -m 80 -h 150
purge_haplotigs purge -g hifiasm.asm.bp.p_utg.fa -c coverage_stats.csv

#mapping Hi-C to curated assembly
hicstuff pipeline -D -t 4 -o hicstuff/ --genome curated.fasta -m iterative -e DpnII -n data/Hi-C/adineta_pcrfree.end1.fq  data/Hi-C/adineta_pcrfree.end2.fq
samtools cat hicstuff/tmp/*.bam > hicstuff/tmp/hic.bam
samtools sort hicstuff/tmp/hic.bam > hicstuff/tmp/hic_sorted.bam
bamToBed -i hicstuff/tmp/hic_sorted.bam > hicstuff/tmp/hic_sorted.bed
...
```
Here assembly\_graph.fa is the conversion of assembly\_graph.gfa to fasta format.

### Puccina diploid assembly
```
#hifiasm assembly
hifiasm -o hifiasm.asm data/HiFi/SRR14424683.1.fastq -t 8 -m 100000 -l 0

#mapping of Hi-C on assembly
hicstuff pipeline data/Hi-C/SRR14386306.1_1.fastq data/Hi-C/SRR14386306.1_2.fastq -D -e DpnII -g hifiasm.asm.bp.p_utg.fa -m iterative -n -t 4 --force

#graphunzip
python graphunzip.py HiC-IM -g hifiasm.asm.bp.p_utg.gfa -m hicstuff/abs_fragments_contacts_weighted.txt -F fragments_list.txt -i interactionMatrix
python graphunzip.py unzip -r -g hifiasm.asm.bp.p_utg.gfa -i interactionMatrix -o unzipped_merged.gfa

#hifiasm with embedded Hi-C treatment
hifiasm -o hifiasm_HiC.asm data/HiFi/SRR14424683.1.fastq -t 8 -m 100000 -l 0 --h1 data/Hi-C/SRR14386306.1_1.fastq --h2 data/Hi-C/SRR14386306.1_2.fastq
```

### Adineta diploid assembly
```
#flye assembly
flye --nano-raw data/Nanopore/SRR13348928.fastq --keep-haplotypes -o assembly_flye -t 8

#mapping of Hi-C on assembly
hicstuff pipeline -D -e DpnII -g assembly_graph.gfa -m iterative -n -t 4 --force data/Hi-C/adineta_pcrfree.end1.fq data/Hi-C/adineta_pcrfree.end2.fq

#graphunzip
python graphunzip.py HiC-IM -g assembly_graph.gfa -m hicstuff/abs_fragments_contacts_weighted.txt -F fragments_list.txt -i interactionMatrix
python graphunzip.py unzip -r -g assembly_graph.gfa -i interactionMatrix -o unzipped_merged.gfa -f unzipped_merged.fasta
```
