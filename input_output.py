...
def interactionMatrix(
    hicFile, fragmentList, names, segments, header=True
...
def longReads_interactionsMatrix(
    ...
    for line in f:
        ...
        if ls[5].count(">") + ls[5].count("<") > 1:
            ...
            if (
                not "id:f" in ls[-2]
                or float(ls[-2].split(":")[-1]) > similarity_threshold
            ):
                        ...
                        for c2 in range(c1 + 1, len(contigs)):
                            if contigs[c1] in names and contigs[c2] in names:
                                if names[contigs[c1]] != names[contigs[c2]]:
                                    interactionMatrix[
                                        names[contigs[c1]], names[contigs[c2]]
                                    ] = 10
                                    if c2 == c1 + 1:
                                        allLinks.add(
                                            (
...
def load_gfa(file):
    ...
    for line in gfa_read:
        if line[0] == "S":
            ...
            names[
                s.names[0]
            ] = index  # that is the (strange) way of adding a key to a dict in python
            index += 1
