...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        default="Empty",
        help="""Long reads mapped to the GFA with GraphAligner (GAF format)""",
    )
...
def main():
    ...
    if lrFile is not "Empty":
        ...
        lrInteractionMatrix, lrLinks = io.longReads_interactionsMatrix(
            lrFile, names, segments, similarity_threshold=mm, whole_mapping=wm
        )
    ...
    if interactionMatrix.count_nonzero() == 0:
        print(
            "WARNING: the interaction matrix between contigs is empty. This could be due to having filtered out all information from long reads. If you used --exhaustive I remove all edges, I do nothing elsewhise."
        )
