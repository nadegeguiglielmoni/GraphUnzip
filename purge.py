...
def purge_assembly(segments):
    ...
    refCoverage, multiplicities = determine_multiplicity(segments, reliable_coverage=confidentCoverage)
    ...
    for s, seg in enumerate(segments):
        ...
        for end in range(2):
            ...
            if multiplicities[s] == target_ploidy :#and "edge_1498" in seg.names and end == 1 :
                ...
                unavoidable_segments = {i for i in unavoidable_segments if multiplicities[IDs[i[0].ID]] == target_ploidy}
                ...
                if len(unavoidable_segments) > 0 :
                    ...
                    longestPath = pathlengths.index(max(pathlengths))  #keep only the longest path
...
def unavoidable_neighbors(segment, end, depth, maxdepth, path):
    ...
    for n, neighbor in enumerate(segment.links[end]):
        ...
        if (neighbor, 1-otherEnd) not in path:
            ...
            unavoidablethere = unavoidable_neighbors(neighbor, 1-otherEnd, depth+1, maxdepth, newpath)
...
def find_all_paths(segment, end, unavoidable_segments, seen_contigs):
    ...
    for n, neighbor in enumerate(segment.links[end]):
        ...
        if (neighbor, 1-otherEnd) not in seen_contigs:
            ...
            pathsthere = find_all_paths(neighbor, 1-otherEnd, unavoidable_segments, seen_contigs)
            for i in pathsthere:
                i.add(segment)
            result += pathsthere
