...
def assign_a_chromosome_to_each_contig(
    assemblyFile, queryfiles, fileOut="assign.pickle", chunks=2000
):
    ...
    for query in queryfiles:
        ...
        blastn_cline = NcbiblastnCommandline(
            ...
            evalue=0.000001,
            outfmt=5,
            out="tmp/blast_sol" + query.strip(".fasta") + ".xml",
            task="megablast",
            num_alignments=5,
            qcov_hsp_perc=90,
            num_threads=3,
        ...
        #             #logging.info("identities ", hsp.identities)
        ...
        #                 logging.info(alignment.title[:-19], " matches to ", query.rstrip('_cut.fasta'))
        #
        #         if nbhits == 0 :
        #             nowhere += 1
        #         total += 1

        # logging.info('In assign_a_chromosome_to_each_contig, haven\'t managed to map', nowhere/total*100, ' % of the chunks')
...
def check_phasing(
    assigned, fastaFile, fileOut
):  # the contigs of the fasta file should be merged with '_' between the names
    fo = open(fileOut, "w")
        ...
        if ">" in line[0]:
            ...
            for contig in listOfContigs:
                if contig not in assigned:
                    # logging.info('There is a problem in the way the contigs are named, possibly if they contain _ or - in their names')
                    r = 0
                else:
                    if assigned[contig] != []:
                        if first:
                            ...
                            if len(chromosomes) == 0:
                                ...
                                phasingErrors += 1
                        logging.info(contig, " ", ass[contig])
                        fo.write(contig + " " + str(ass[contig]) + "\n")

            # logging.info('Contig ', listOfContigs, ' is in chromosome ', chromosomes)
...
# logging.info(queryfiles) #give the value outputted here to query files in the future
# logging.info('Done cutting')
...
assign_a_chromosome_to_each_contig(
    "potato/stuberosum.hifiasm_l0.p_utg.fasta",
    queryfiles,
    "potato/assign.pickle",
    chunks=10000,
)
...
# logging.info(assigned['utg000030l'])
# assigned['881'],
# assigned['854'],
# assigned['776'])
