...
def determine_multiplicity(
    segments,
    supported_links=sparse.lil_matrix((0, 0)),
    ...
    for s in segments:
        links = s.get_links()
        if (
            len(links[0]) <= 1 and len(links[1]) <= 1
        ):  # if the contig has less than 1 neighbor at each end we can suppose it is haploid
    ...
    for s in segments:
        ...
        if (
            len(links[0]) <= 1
            and len(links[1]) <= 1
            and (round(s.depths[0] / refCoverage) <= 1 or refCoverage == 1)
        ):  # if the contig has 1 neighbor at each end we can suppose it is haploid
    ...
    while unchanged < len(segments):
        if computed_multiplicity[s] == 0:
            ...
            confidence = False
            if (
                len(segments[s].links[0]) > 0
                and all(
                    [computed_multiplicity[IDs[i.ID]] > 0 for i in segments[s].links[0]]
                )
                and all(
                    [
                        len(i.links[segments[s].otherEndOfLinks[0][j]]) == 1
                        for j, i in enumerate(segments[s].links[0])
                    ]
                )
            ):
                new_multiplicity1 = sum(
                    [computed_multiplicity[IDs[i.ID]] for i in segments[s].links[0]]
                )

            if (
                len(segments[s].links[1]) > 0
                and all(
                    [computed_multiplicity[IDs[i.ID]] > 0 for i in segments[s].links[1]]
                )
                and all(
                    [
                        len(i.links[segments[s].otherEndOfLinks[1][j]]) == 1
                        for j, i in enumerate(segments[s].links[1])
                    ]
                )
            ):
                new_multiplicity2 = sum(
                    [computed_multiplicity[IDs[i.ID]] for i in segments[s].links[1]]
                )

            if (
                new_multiplicity1 == new_multiplicity2
            ):  # in this case the confidence is very high
                new_multiplicity = new_multiplicity1
                confidence = True
            else:
                new_multiplicity = max(
                    new_multiplicity1, new_multiplicity2
                )  # that includes the case where both are > 0, in which case we are not confident

            # if 'edge_224' in segments[s].names :
            #      print("Looking at 224 and ", new_multiplicity)
            if new_multiplicity > 0 and (
                segments[s].depths[0] / refCoverage > new_multiplicity / 1.5
                or confidence
                or refCoverage == 1
            ):
            ...
            if new_multiplicity1 == new_multiplicity and new_multiplicity > 0:
                for n, neighbor in enumerate(segments[s].links[0]):
                    supported_links[
                        2 * IDs[segments[s].ID],
                        2 * IDs[neighbor.ID] + segments[s].otherEndOfLinks[0][n],
                    ] = computed_multiplicity[IDs[neighbor.ID]]
                    supported_links[
                        2 * IDs[neighbor.ID] + segments[s].otherEndOfLinks[0][n],
                        2 * IDs[segments[s].ID],
                    ] = computed_multiplicity[IDs[neighbor.ID]]
            if new_multiplicity2 == new_multiplicity and new_multiplicity > 0:
                for n, neighbor in enumerate(segments[s].links[1]):
                    supported_links[
                        2 * IDs[segments[s].ID] + 1,
                        2 * IDs[neighbor.ID] + segments[s].otherEndOfLinks[1][n],
                    ] = computed_multiplicity[IDs[neighbor.ID]]
                    supported_links[
                        2 * IDs[neighbor.ID] + segments[s].otherEndOfLinks[1][n],
                        2 * IDs[segments[s].ID] + 1,
            ...
            for end in range(2):
                if all(
                    [
                        len(i.links[segments[s].otherEndOfLinks[end][j]]) == 1
                        for j, i in enumerate(segments[s].links[end])
                    ]
                ):
                    number0 = 0
                    index0 = 0
                    for n, neighbor in enumerate(segments[s].links[end]):
                        if computed_multiplicity[IDs[neighbor.ID]] == 0:
                            ...
                            index0 = n
                    if number0 == 1:
                        new_multiplicity = computed_multiplicity[s] - sum(
                            [
                                computed_multiplicity[IDs[i.ID]]
                                for i in segments[s].links[end]
                            ]
                        )

                        if new_multiplicity > 0 and (
                            segments[s].depths[0] / refCoverage
                            >= new_multiplicity / 1.5
                            or refCoverage == 1
                        ):
                            computed_multiplicity[
                                IDs[segments[s].links[end][index0].ID]
                            ] = new_multiplicity
                            # print("salut, ", s+1, " ", IDs[segments[s].links[end][index0].ID]+1, " ", new_multiplicity)
                            unchanged = -1

                            supported_links[
                                2 * IDs[segments[s].ID] + end,
                                2 * IDs[segments[s].links[end][index0].ID]
                                + segments[s].otherEndOfLinks[end][index0],
                            ] = new_multiplicity
                            supported_links[
                                2 * IDs[segments[s].links[end][index0].ID]
                                + segments[s].otherEndOfLinks[end][index0],
        ...
        for s, seg in enumerate(segments):
            if computed_multiplicity[s] > 0:
                for end in range(2):
                    if all(
                        [
                            len(i.links[seg.otherEndOfLinks[end][j]]) == 1
                            for j, i in enumerate(seg.links[end])
                        ]
                    ):
                        ...
                        if covTot != 0:
                            for n, neighbor in enumerate(seg.links[end]):
                                if computed_multiplicity[IDs[neighbor.ID]] == 0:
                                    computed_multiplicity[IDs[neighbor.ID]] = round(
                                        computed_multiplicity[s]
                                        * neighbor.depths[0]
                                        / covTot
                                    )
                                    if (
    ...
    sortedContigs = [
        IDs[i.ID] for i in sorted(segments, key=lambda x: x.length, reverse=True)
    ]
    ...
    while sortedContigidx >= 0:
        if computed_multiplicity[sortedContigidx] == 0:
            ...
            for n, neighbor in enumerate(segments[sortedContigidx].links[0]):
                if (
                    len(neighbor.links[segments[sortedContigidx].otherEndOfLinks[0][n]])
                    == 1
                ):
                    minLeft += computed_multiplicity[IDs[neighbor.ID]]
            for n, neighbor in enumerate(segments[sortedContigidx].links[1]):
                if (
                    len(neighbor.links[segments[sortedContigidx].otherEndOfLinks[1][n]])
                    == 1
                ):
                    minRight += computed_multiplicity[IDs[neighbor.ID]]

            computed_multiplicity[sortedContigidx] = max(
                1, minLeft, minRight
            )  # choosing 1 there means that computed_multiplicity is a minimum multiplicity
            if segments[sortedContigidx].length > 5000 and reliable_coverage:
                computed_multiplicity[sortedContigidx] = max(
                    computed_multiplicity[sortedContigidx],
                    round(segments[sortedContigidx].depth / refCoverage),
                )
            propagate_multiplicity(
                computed_multiplicity,
                segments,
...
def propagate_multiplicity(
    multiplicities, segments, IDs, contigIdx, supported_links, refCoverage
):
    ...
    for end in range(2):
        for n, neighbor in enumerate(segments[contigIdx].links[end]):
            ...
            if multiplicities[IDs[neighbor.ID]] == 0:
                if all(
                    [multiplicities[IDs[i.ID]] > 0 for i in neighbor.links[neighborEnd]]
                ) and all(
                    [
                        len(
                            neighbor.links[neighborEnd][i].links[
                                neighbor.otherEndOfLinks[neighborEnd][i]
                            ]
                        )
                        == 1
                        for i in range(len(neighbor.links[neighborEnd]))
                    ]
                ):
                    multiplicities[IDs[neighbor.ID]] = sum(
                        [multiplicities[IDs[i.ID]] for i in neighbor.links[neighborEnd]]
                    )

                    for n2, neighbor2 in enumerate(neighbor.links[neighborEnd]):
                        supported_links[
                            2 * IDs[neighbor.ID] + neighborEnd,
                            2 * IDs[neighbor2.ID]
                            + neighbor.otherEndOfLinks[neighborEnd][n2],
                        ] = multiplicities[IDs[neighbor2.ID]]
                        supported_links[
                            2 * IDs[neighbor2.ID]
                            + neighbor.otherEndOfLinks[neighborEnd][n2],
                            2 * IDs[neighbor.ID] + neighborEnd,
                    ...
                    propagate_multiplicity(
                        ...
                        supported_links,
                        refCoverage,
                    )  # recursive call : found a multiplicity, move on

            elif multiplicities[IDs[neighbor.ID]] > 0:
                if all(
                    [
                        len(i.links[neighbor.otherEndOfLinks[neighborEnd][j]]) == 1
                        for j, i in enumerate(neighbor.links[neighborEnd])
                    ]
                ):
                    number0 = 0
                    index0 = 0
                    for n2, neighbor2 in enumerate(neighbor.links[neighborEnd]):
                        if multiplicities[IDs[neighbor2.ID]] == 0:
                            ...
                            index0 = n2
                    if number0 == 1:
                        new_multiplicity = multiplicities[IDs[neighbor.ID]] - sum(
                            [
                                multiplicities[IDs[i.ID]]
                                for i in neighbor.links[neighborEnd]
                            ]
                        )

                        if new_multiplicity > 0 and (
                            neighbor.links[neighborEnd][index0].depths[0] / refCoverage
                            >= new_multiplicity / 1.5
                            or refCoverage == 1
                        ):
                            multiplicities[
                                IDs[neighbor.links[neighborEnd][index0].ID]
                            ] = new_multiplicity
                            ...
                            supported_links[
                                2 * IDs[neighbor.ID] + neighborEnd,
                                2 * IDs[neighbor.links[neighborEnd][index0].ID]
                                + neighbor.otherEndOfLinks[neighborEnd][index0],
                            ] = new_multiplicity
                            supported_links[
                                2 * IDs[neighbor.links[neighborEnd][index0].ID]
                                + neighbor.otherEndOfLinks[neighborEnd][index0],
                ...
                logging.debug("debug WARNING: negative multiplicity")
    ...
    if refCoverage != 1:
        ...
        for end in range(2):
            ...
            ) and not any([i.depth == 0 for i in seg.links[end]]):
                covTot = sum([i.depths[0] for i in seg.links[end]])
                tot = 0
                ...
                for n, neighbor in enumerate(seg.links[end]):
                    if multiplicities[IDs[neighbor.ID]] == 0:
                        new_multiplicity = max(
                            min(
                                round(
                                    multiplicities[contigIdx]
                                    * neighbor.depths[0]
                                    / covTot
                                ),
                                multiplicities[contigIdx] - len(seg.links[end]) + 1,
                            ),
                            1,
                        )
                        ...
                        if (
                            round(
                                multiplicities[contigIdx] * neighbor.depths[0] / covTot
                            )
                            > 0
                        ):
                            found = True
                            supported_links[
                                2 * IDs[seg.ID] + end,
                                2 * IDs[neighbor.ID] + seg.otherEndOfLinks[end][n],
                            ] = new_multiplicity
                            supported_links[
                                2 * IDs[neighbor.ID] + seg.otherEndOfLinks[end][n],
                                2 * IDs[seg.ID] + end,
