...
def parse_args_command():
    ...
    parser.add_argument(
        "command",
        help="Either unzip (untangle the GFA file), purge (retain only haploid contigs), extract (extract haploid assembly with a close reference genome), HiC-IM (to prepare Hi-C data) or linked-reads-IM (to prepare linked reads data)",
    )
