...
wat += (
    ...
    + "".join(
        [
            (
                "\n===============================================================================================\n\n  "
                if code == "file_alignments_read" or code == "sam"
                else "  "
            )
            + (code + " ").ljust(25, "-")
            + description
...
class read:
    ...
    def __init__(self, f, fields=False, decompressor=False):
        ...
        if decompressor:
            if type(decompressor) is str:
                if decompressor != "internal" and "{}" not in decompressor:
                    raise PybamError(
        ...
        def generator():
            ...
            if magic == b"BAM\1":
            ...
            ):  # The user has passed us compressed gzip/bgzip data, which is typical for a BAM file
                # use custom decompressor if provided:
                if decompressor is not False and decompressor != "internal":
                    if type(f) is str:
                        self._subprocess = subprocess.Popen(
                            decompressor.replace("{}", f),
                            shell=True,
                ...
                # else look for pigz or gzip:
                    ...
                    while raw_data:
                        ...
                        try:
                            more_bs = (
                                bs + unpack("<H", raw_data[bs + 16 : bs + 18])[0] + 1
                            )
                            internal_cache.append(
                                decompress(raw_data[bs + 18 : more_bs - 8], -15)
                            )
                            bs = more_bs
                            ...
                            while header_size - 12 < extra_len:
                                ...
                                subfield_data = raw_data[
                                    bs + 16 : bs + 16 + subfield_len
                                ]
                            ...
                            bs = (
                                bs + 16 + subfield_len + block_size - extra_len - 19 + 8
                            )
                            zipped_data = header_data + raw_data + crc_data
                            internal_cache.append(
                                decompress(zipped_data, 47)
        ...
        for _ in range(number_of_reference_sequences):
            ...
            while len(header_cache) < p_to:
                header_cache += next(self._generator)
            self.file_chromosome_lengths[self.file_chromosomes[-1]] = unpack(
                "<l", header_cache[p_from:p_to]
            )[0]
        ...
        def compile_parser(self, fields):
            ...
            if (
                "sam_cigar_string" in dependencies
                or "sam_cigar_list" in dependencies
                or "bam_cigar" in dependencies
            ):
                if end_of_qname:
                    pass
                else:
    ...
    @property
    def bam_pos(self):
        return self.bam[8:12]

    @property
    def bam_l_read_name(self):
        return self.bam[12:13]

    @property
    def bam_mapq(self):
        return self.bam[13:14]

    @property
    def bam_bin(self):
        return self.bam[14:16]

    @property
    def bam_n_cigar_op(self):
        return self.bam[16:18]

    @property
    def bam_flag(self):
        return self.bam[18:20]

    @property
    def bam_l_seq(self):
        return self.bam[20:24]

    @property
    def bam_next_refID(self):
        return self.bam[24:28]

    @property
    def bam_pnext(self):
        return self.bam[28:32]

    @property
    def bam_tlen(self):
        return self.bam[32:36]

    @property
    def bam_qname(self):
        return self.bam[36 : self._end_of_qname]

    @property
    def bam_cigar(self):
        return self.bam[self._end_of_qname : self._end_of_cigar]

    @property
    def bam_seq(self):
        return self.bam[self._end_of_cigar : self._end_of_seq]

    @property
    def bam_qual(self):
        return self.bam[self._end_of_seq : self._end_of_qual]

    @property
    def bam_tags(self):
        return self.bam[self._end_of_qual :]

    @property
    def sam_refID(self):
        return unpack("<i", self.bam[4:8])[0]

    @property
    def sam_pos0(self):
        return unpack("<i", self.bam[8:12])[0]

    @property
    def sam_l_read_name(self):
        return unpack("<B", self.bam[12:13])[0]

    @property
    def sam_mapq(self):
        return unpack("<B", self.bam[13:14])[0]

    @property
    def sam_bin(self):
        return unpack("<H", self.bam[14:16])[0]

    @property
    def sam_n_cigar_op(self):
        return unpack("<H", self.bam[16:18])[0]

    @property
    def sam_flag(self):
        return unpack("<H", self.bam[18:20])[0]

    @property
    def sam_l_seq(self):
        return unpack("<i", self.bam[20:24])[0]

    @property
    def sam_next_refID(self):
        return unpack("<i", self.bam[24:28])[0]

    @property
    def sam_pnext0(self):
        return unpack("<i", self.bam[28:32])[0]

    @property
    def sam_tlen(self):
        return unpack("<i", self.bam[32:36])[0]

    @property
    def sam_qname(self):
        return self.bam[
            36 : self._end_of_qname - 1
        ].decode()  # -1 to remove trailing NUL byte

    @property
    def sam_cigar_list(self):
        return [
            (cig >> 4, cigar_codes[cig & 0b1111])
            for cig in array("I", self.bam[self._end_of_qname : self._end_of_cigar])
        ]

    @property
    def sam_cigar_string(self):
        return "".join(
            [
                str(cig >> 4) + cigar_codes[cig & 0b1111]
                for cig in array("I", self.bam[self._end_of_qname : self._end_of_cigar])
            ]
        )
    ...
    def sam_seq(self):
        return "".join(
            [
                dna_codes[dna >> 4] + dna_codes[dna & 0b1111]
                for dna in array("B", self.bam[self._end_of_cigar : self._end_of_seq])
            ]
        )[
            : self.sam_l_seq
        ]  # As DNA is 4 bits packed 2-per-byte, there might be a trailing '0000', so we can either
        ...
        while offset != len(self.bam):
            ...
            if tag_type == "Z":
                ...
                offset_end = (
                    ...
                    + 8
                    + (
                        unpack("<i", self.bam[offset + 4 : offset + 8])[0]
                        * py4py[self.bam[offset + 3]]
                    )
                )
                tag_data = array(
                    self.bam[offset + 3], self.bam[offset + 8 : offset_end]
                )
            else:
                print(
    ...
    def sam_tags_string(self):
        return "\t".join(
            A
            + ":"
            + ("i" if B in "cCsSI" else B)
            + ":"
            + (
                (C.typecode + "," + ",".join(map(str, C)))
                if type(C) == array
                else str(C)
            )
            for A, B, C in self.sam_tags_list
        )

    ## BONUS methods - methods that mimic how samtools works.
    @property
    def sam_pos1(self):
        return 0 if self.sam_pos0 < 0 else self.sam_pos0 + 1

    @property
    def sam_pnext1(self):
        return 0 if self.sam_pnext0 < 0 else self.sam_pnext0 + 1

    @property
    def sam_rname(self):
        return "*" if self.sam_refID < 0 else self.file_chromosomes[self.sam_refID]

    @property
    def sam_rnext(self):
        return (
            "*"
            if self.sam_next_refID < 0
            else self.file_chromosomes[self.sam_next_refID]
        )
    ...
    @property
    def _end_of_qname(self):
        return (
            self.sam_l_read_name + 36
        )  # fixed-length stuff at the beginning takes up 36 bytes.

    @property
    def _end_of_cigar(self):
        return self._end_of_qname + (4 * self.sam_n_cigar_op)  # 4 bytes per n_cigar_op

    @property
    def _end_of_seq(self):
        return self._end_of_cigar + (-((-self.sam_l_seq) // 2))  # {blurgh}

    @property
    def _end_of_qual(self):
        return self._end_of_seq + self.sam_l_seq  # qual has the same length as seq
