...
class Segment:
    def __init__(
        self,
        segNamesOfContig,
        segOrientationOfContigs,
        segLengths,
        segInsideCIGARs=None,
        segLinks=[[], []],
        segOtherEndOfLinks=[[], []],
        segCIGARs=[[], []],
        lock=False,
        ...
        self._namesOfContigs = (
            segNamesOfContig.copy()
        )  # names are strings with which sequences are described in the GFA
        self._orientationOfContigs = (
            segOrientationOfContigs.copy()
        )  # 1 being '+' orientation, 0 the '-' orientation
        self._lengths = segLengths.copy()
        self._insideCIGARs = segInsideCIGARs.copy()
        if readCoverage != []:
            self._depths = (
                readCoverage.copy()
            )  # to keep in mind the read coverage of the contigs
        else:
            self._depths = [1 for i in range(len(self._lengths))]

        self._copiesOfContigs = [-1] * len(
            segNamesOfContig
        )  # this is used exclusively while exporting, to indicate which copy of which contig is in the segment (copy 0/ copy 1 / copy 2 ...)
        ...
        lists_keyed = [
            [
                (segLinks[0][i], segOtherEndOfLinks[0][i], segCIGARs[0][i])
                for i in range(len(segLinks[0]))
            ],
            [
                (segLinks[1][i], segOtherEndOfLinks[1][i], segCIGARs[1][i])
                for i in range(len(segLinks[1]))
            ],
        ]
        lists_keyed[0].sort(key=lambda x: x[0].ID)
        lists_keyed[1].sort(key=lambda x: x[0].ID)
        self._links = [
            [i[0] for i in lists_keyed[0]],
            [i[0] for i in lists_keyed[1]],
        ]  # two lists of segments with which the segment is linked, at the left end and at the right end
        self._otherEndOfLinks = [
            [i[1] for i in lists_keyed[0]],
            [i[1] for i in lists_keyed[1]],
        ]  # for each link, indicates the side of the other segment on which the link arrives
        self._CIGARs = [
            [i[2] for i in lists_keyed[0]],
            [i[2] for i in lists_keyed[1]],
        ]  # for each link, indicates the CIGAR string found in the GFA

        self._freezed = [
            False,
            False,
        ]  # do not duplicate from one end if frozen at this end
        self._locked = (
            lock  # That is to duplicate a contig only once in each merge_contigs
        )
    ...
    def full_name(self):
        return "_".join(
            [
                self._namesOfContigs[i] + "-" + str(self._copiesOfContigs[i])
                for i in range(len(self._namesOfContigs))
            ]
        )
    ...
    def divide_depths(
        self, n
    ):  # when duplicating a segment, you need to lower the coverage of all replicas
        for i in range(len(self._depths)):
            self._depths[i] /= n

    def multiply_end_depths(
        self, n, end, numberOfContigs
    ):  # this fucntion is useful when merging a wrongly duplicated dead end
        for co in range(numberOfContigs):
            self._depths[end * (len(self._depths) - 1) + (-2 * end + 1) * co]

    def length1(
        self,
    ):  # use this function to set length of a segment to 1 (instead of 0, mostly)
    ...
    freezed = property(
        get_freezed
    )  # segment is freezed if comparison of links was inconclusive
    locked = property(
        get_locked, set_locked
    )  # segment is locked if it was already duplicated in merge_contigs and that it should not be for a second time
    ...
    def interaction_with_contigs(
        ...
        for co, contig in enumerate(self.names):
            for endself in range(2):
                posselfend = (
                    posself
                    + abs(self.orientations[co] + endself - 1) * self.lengths[co]
                )
                if abs(selfend * self.length - posselfend) < halfself or True:
                    ...
                    for c, contigInSegment in enumerate(segment.names):
                        for endsegment in range(2):
                            posneighborend = (
                                posneighbor
                                + abs(segment.orientations[c] + endsegment - 1)
                                * segment.lengths[c]
                            )

                            if (
                                abs(segmentend * segment.length - posneighborend)
                                < halfneighbor
                                or True
                            ):
                                ...
                                if (
                                    contig not in commonContigs
                                    and copiesnumber[contigInSegment] <= bestSignature
                                ):
    ...
    def add_link_from_GFA(
        self, GFAline, names, segments, leftOrRight
    ):  # leftOrRight = 0 when the segment is at the beginning of a link (left of a GFA line), 1 otherwise
        l = GFAline.strip("\n").split("\t")
        ...
        if l[0] != "L":
            ...
            if leftOrRight == 0 and o1 == 0:
                ...
                if find_this_link(
                    ...
                    1 - o2,
                    self._links[0],
                    self._otherEndOfLinks[0],
                ) == -1 or (segments[names[l[3]]].ID == self._id and 1 - o2 == 0):
                ...
                if find_this_link(
                    ...
                    1 - o2,
                    self._links[1],
                    self._otherEndOfLinks[1],
                ) == -1 or (segments[names[l[3]]].ID == self._id and 1 - o2 == 1):
                    index = index_at_which_new_link_should_be_inserted(
                        segments[names[l[3]]],
                        self._links[1],
                        1 - o2,
                ...
                if find_this_link(
                    segments[names[l[1]]], o1, self._links[0], self._otherEndOfLinks[0]
                ) == -1 or (segments[names[l[1]]].ID == self._id and o1 == 0):
                    index = index_at_which_new_link_should_be_inserted(
                        segments[names[l[1]]],
                        self._links[0],
                        o1,
                ...
                if find_this_link(
                    segments[names[l[1]]], o1, self._links[1], self._otherEndOfLinks[1]
                ) == -1 or (segments[names[l[1]]].ID == self._id and o1 == 1):
                    index = index_at_which_new_link_should_be_inserted(
                        segments[names[l[1]]],
                        self._links[1],
                        o1,
    ...
    def add_end_of_link(self, endOfSegment, segment2, endOfSegment2, CIGAR="*"):
        ...
        index = index_at_which_new_link_should_be_inserted(
            ...
            endOfSegment2,
            self._otherEndOfLinks[endOfSegment],
        )
    ...
    def add_a_bunch_of_end_of_links(
        self, endOfSegment, listOfSegmentsToAdd, listOfEndOfSegmentsToAdd, CIGARsToAdd
    ):
        ...
        while indexOfSegmentToAdd < len(listOfSegmentsToAdd) and indexOfLinks < len(
            self._links[endOfSegment]
        ):
            if (
                listOfSegmentsToAdd[indexOfSegmentToAdd].ID
                < self._links[endOfSegment][indexOfLinks].ID
            ):
            ...
            # elsewhise the link goes toward the same segment, but maybe not the same end of this segment
                ...
                listOfEndOfSegmentsToAdd[indexOfSegmentToAdd]
                < self._otherEndOfLinks[endOfSegment][indexOfLinks]
            ):
                newLinks.append(listOfSegmentsToAdd[indexOfSegmentToAdd])
                newEndOfLinks.append(listOfEndOfSegmentsToAdd[indexOfSegmentToAdd])
                newCIGARs.append(CIGARsToAdd[indexOfSegmentToAdd])
    ...
    def break_contig(self, axis):
        newSegment1 = Segment(
            self._namesOfContigs[:axis],
            self._orientationOfContigs[:axis],
            self._lengths[:axis],
            self._insideCIGARs[: axis - 1],
            [self._links[0], []],
            [self._otherEndOfLinks[0], []],
            [self._CIGARs[0], []],
            readCoverage=self._depths[:axis],
        )
    ...
    def flatten(self, replicas):
        if self not in self._links[0]:
            ...
            for i in range(replicas):
                ...
                newinsideCIGARs += [
                    self._CIGARs[0][self._links[0].index(self)]
                ] + self._insideCIGARs
...
def merge_two_segments(segment1, endOfSegment1, segment2, listOfSegments):
    ...
    endOfSegment2 = segment1.otherEndOfLinks[endOfSegment1][
        segment1.links[endOfSegment1].index(segment2)
    ]
    ...
    if (
        orientation1 == -1
    ):  # then change the orientation of all the contigs within the segment, since the segment will be mirrored in the new supersegment
        orientationOfContigs1 = [1 - i for i in orientationOfContigs1]
    if orientation2 == -1:
        orientationOfContigs2 = [1 - i for i in orientationOfContigs2]

    CIGAR = segment1.CIGARs[endOfSegment1][
        segment1.links[endOfSegment1].index(segment2)
    ]
    ...
    newSegment = Segment(
        ...
        segment1.insideCIGARs[::orientation1]
        + [CIGAR]
        + segment2.insideCIGARs[::orientation2],
        segLinks=[segment1.links[1 - endOfSegment1], segment2.links[1 - endOfSegment2]],
        segOtherEndOfLinks=[
            segment1.otherEndOfLinks[1 - endOfSegment1],
            segment2.otherEndOfLinks[1 - endOfSegment2],
        ],
        segCIGARs=[
            segment1.CIGARs[1 - endOfSegment1],
    ...
    for n, neighbor in enumerate(newSegment.links[0]):
        neighbor.add_end_of_link(
            newSegment.otherEndOfLinks[0][n],
            newSegment,
            0,
            CIGAR=newSegment.CIGARs[0][n],
        )
        if (
            neighbor.ID == segment2.ID
            and newSegment.otherEndOfLinks[0][n] == 1 - endOfSegment2
        ):  # check if the new contig should loop back on itself
...
def find_this_link(
    segment, endOfSegment, listOfLinks, listOfEndsOfLinks, warning=False
):
    lo = 0
    hi = len(listOfLinks)
...
def index_at_which_new_link_should_be_inserted(
    segment, listOfSegments, endOfLink, listOfEndOfLinks
):
    lo = 0
    ...
    while lo < hi:
        mid = (lo + hi) // 2
        if segment.ID < listOfSegments[mid].ID or (
            segment.ID == listOfSegments[mid].ID and endOfLink < listOfEndOfLinks[mid]
        ):
            hi = mid
        else:
            lo = mid + 1
    ...
    while (
        lo < len(listOfSegments)
        and listOfSegments[lo].ID == segment.ID
        and endOfLink == listOfEndOfLinks[lo]
    ):
        lo += 1
    return lo
            ...
            for n in range(len(segment.links[endOfSegment]) - 1):
                if (
                    ...
                    logging.info(
                        ...
                        segment.names,
                        " : ",
                        [s.ID for s in segment.links[endOfSegment]],
                    )
        ...
        for endOfSegment in range(2):
            for n1 in range(len(segment.links[endOfSegment]) - 1):
                ...
                for n2 in range(n1 + 1, len(segment.links[endOfSegment])):
                    if (
                        segment.links[endOfSegment][n1].ID
                        == segment.links[endOfSegment][n2].ID
                        and segment.otherEndOfLinks[endOfSegment][n1]
                        == segment.otherEndOfLinks[endOfSegment][n2]
                        and segment.links[endOfSegment][n1].ID != segment.ID
                    ):
                        segment.links[endOfSegment][n2].remove_end_of_link(
                            segment.otherEndOfLinks[endOfSegment][n2],
                            segment,
                            endOfSegment,
                        )
