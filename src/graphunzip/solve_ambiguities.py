...
def intensity_of_interactions(
    ...
    listOfTouchingEnds = [
        segment.otherEndOfLinks[endOfSegment][i] for i in listOfNeighborIndices
    ]
    ...
    commonContigs = compute_commonContigs2(
        segment, endOfSegment, listOfNeighborIndices, 2000000
    )
    ...
    for c in candidatesSegments:
        absoluteScore, relativeScore, depthHere = c.interaction_with_contigs(
            segment,
            interactionMatrix,
            names,
            copiesnumber,
...
def propagate_vicinity(segment, endOfSegment, vicinityContigs, depth, limitDepth):
    if depth > limitDepth:
        ...
        for name in segment.names:
            vicinityContigs.add(
                name + "$:" + str(endOfSegment) + "$:" + str(segment.ID)
            )
        for n, neighbor in enumerate(segment.links[1 - endOfSegment]):
            if (
                neighbor.names[0]
                + "$:"
                + str(segment.otherEndOfLinks[1 - endOfSegment][n])
                + "$:"
                + str(neighbor.ID)
                not in vicinityContigs
            ):
                propagate_vicinity(
                    neighbor,
                    segment.otherEndOfLinks[1 - endOfSegment][n],
                    vicinityContigs,
...
def compute_commonContigs(segment, candidatesSegments, listOfTouchingEnds, depth):
    ...
    if depth == 2:
        ...
        for c in candidatesSegments:
            if all(
                [elem in commonContigs for elem in c.names]
            ):  # this is not good, commoncontigs is too big
        ...
        if neighborOfNeighborActivated:
            return (
                commonContigs,
                True,
            )  # the True value is to signifie that neighbors of neighbors were used
    ...
    return (
        commonContigs,
        False,
    )  # the False value is to signifie that neighbors of neighbors were not used
        ...
        for endOfSegment in range(2):
            ...
            for n, neighbor in enumerate(segment.links[endOfSegment]):
                ...
                link = (
                    ...
                    (segment.orientations[-endOfSegment] == endOfSegment),
                    neighbor.names[-endOfNeighbor],
                    (neighbor.orientations[-endOfNeighbor] == endOfNeighbor),
                )
                linkb = (link[2], link[3], link[0], link[1])

                if (
                    not link in lr_links and not linkb in lr_links
                ):  # then the link is not confirmed by long reads
