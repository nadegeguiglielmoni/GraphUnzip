...
def solve_with_HiC(segments, interactionMatrix, names, haploidContigs = [], copiesnumber={}, confidentCoverage=True, noisy=False, verbose = False, haploid=False):
    ...
    number_of_ends_of_all_contigs = {i.names[0] : (len(i.links[0]), len(i.links[1])) for i in segments} #useful only if noisy
    ...
    while go_on > 0 and limit_counter < limit:
        ...
        for s in segments :
            if '10' in s.names :
                    print("10 is haploid: ", s.full_name() in haploidContigsNames, ", in contig : ", s.full_name())
        ...
        untangled_paths, list_of_unused_contigs = find_paths(contacts, segments, list_of_knots, solvedKnots, haploidContigsNames, haploidContigs, normalInteractions, names, confidentCoverage, refCoverage, verbose)
    ...
    if haploid:
        ...
        for se, s in enumerate(segments):    
            if len(s.links[0]) == 0 and len(s.links[1]) == 0 :
                if len(s.names) == 1 and (number_of_ends_of_all_contigs[s.names[0]][0] > 0 or number_of_ends_of_all_contigs[s.names[0]][1] > 0) and s.depth < 0.7*refCoverage:
                    s.cut_all_links()
                    toDelete += [se]
...
def determine_haploid_contigs(segments, names, confidentCoverage, haploid, noisy, haploidContigs = []):
    ...
    if not haploid:
        ...
        #print("Multiplicity of 1501: ", multiplicities[names["edge_1501"]])
        #now mark as haploid the contigs with multiplicity ploidy 
        for s, segment in enumerate(segments):
            
            rightdepth = (segment.depth/refCoverage > 0.75 * target_ploidy and segment.depth/refCoverage < 1.25 * target_ploidy)
            if multiplicities[s] == target_ploidy or (segment.length > 10000 and rightdepth) :
...
def determine_list_of_knots(segments, haploidContigs, haploidContigsNames, interactionMatrix, names, verbose = False) :
    ...
    for end in range(len(listOfNeighbors)) :
        ...
        setOfNeighbors = set(find_neighbors(haploidContigs[end//2], end%2, segmentsAlreadyTraversed, haploidContigsNames, 1, 100)) 
            ...
            for neighbor in listOfNeighbors[end] :
                ...
                for name1 in haploidContigs[end//2].names :
                    for name2 in haploidContigs[neighbor//2].names :
                        total += interactionMatrix[names[name1]*2+1, names[name2]*2+neighbor%2]
                interactions += [ total ]
    ...
    # print("Reliable lkf: ", haploidContigsNames)
    ...
    listOfKnots = []
    knotOfContig = [-1 for i in range(len(haploidContigs)*2)] #associates to each end of contig its knot
        ...
        for end in range(2) :
            ...
            if not endOfSegmentAlreadySeen[se*2+end] : #means we're looking at a new knot
                ...
                for element in range(0, len(knot)-1):
                    if knot[element]%2 == 0 and knot[element+1] == knot[element]+1: #both ends are here !!
    ...
    #         print("Edge 1920 in this knot : ", [haploidContigs[i//2].names for i in knot])  
        ...
        if len(knot) > 1  :
            ...
            for e, end in enumerate(knot) :
                ...
                if len(knot) == 2: #then no need to compute complicated bridges
                    ...
                    if len(interactions) == 0: #can appear in really rare cases where a contig has one neighbor but not reciprocally because of limits in recursion depth (so contig ends up alone in his knot)
                        interactions = [-1]
                    ...
                    if  'edge_1808' in haploidContigs[end//2].names : #and end == 1: #or 'edge_129' in haploidContigs[end//2].names or 'edge_290' in haploidContigs[end//2].names or 'edge_289' in haploidContigs[end//2].names:
...
def find_paths(contacts, segments, knots, solvedKnots, haploidContigsNames, haploidContigs, interactionMatrix, names, confidentCoverage, refDepth, verbose = False) :
    ...
    #for kn, k in enumerate(solvedKnots):
        ...
        if confidentCoverage or confidentUntangle:
            ...
            repartitionOfContigs, hardlimits = dispatch_contigs(touchedContigs, contacts[k], interactionMatrix, names, haploidContigs, confidentCoverage, refDepth)
...
def dispatch_contigs(touchedContigs, contacts, interactionMatrix, names, haploidContigs, confidentCoverage, refDepth) :
    ...
    for intercontig in touchedContigs.keys() :
        ...
        if confidentCoverage :
            ...
            multiplicity = max([ min(len(intercontig.links[0]), len(intercontig.links[1]))])
        ...
        for p in touchedContigs[intercontig] :
            for subcontig in intercontig.names :
                for subco in haploidContigs[contacts[p][0]//2].names :
                    interaction_with_path[p] += interaction(interactionMatrix, names, subcontig, subco)
...
def untangle_knots(untangled_paths, segments, haploidContigs, confidentCoverage)    :
    ...
    for s, seg in enumerate(newsegments) :
        if '10' in seg.names :
            print("Here is 10 : ", (seg in haps or seg not in pastSegments) and (len(seg.links[0]) == 1 or len(seg.links[1]) == 1 or seg.length > min(np.min([i.length for i in seg.links[0]], initial=0),np.min([i.length for i in seg.links[1]], initial=0))))
        if (seg in haps or seg not in pastSegments) and (len(seg.links[0]) == 1 or len(seg.links[1]) == 1 or seg.length > min(np.min([i.length for i in seg.links[0]], initial=0),np.min([i.length for i in seg.links[1]], initial=0))) : #if seg is not in pastsegment, it has been created thus is haploid (if it is long enough or has 1 link at one of its ends)
...
def interaction(interactionMatrix, names, contig1, contig2) :
    ...
    return interactionMatrix[names[contig1]*2, names[contig2]*2] + interactionMatrix[names[contig1]*2+1, names[contig2]*2] + interactionMatrix[names[contig1]*2, names[contig2]*2+1] + interactionMatrix[names[contig1]*2+1, names[contig2]*2+1]
...
def duplicate_multiploid_contigs(segments, haploidContigs, haploidContigsNames):
    ...
    for s, seg in enumerate(segments):
        ...
        # print("Looking at segment ", seg.names, " ", seg.ID)
                ...
                if not duplicated and sum([multiplicities[IDs[i.ID]] for i in seg.links[end]]) == multiplicities[s] and all([multiplicities[IDs[i.ID]]>0 for i in seg.links[end]]) and seg.ID not in [i.ID for i in seg.links[end]] and len(seg.links[end]) >1: #nice duplication here
                    ...
                    for n, neighbor in enumerate(seg.links[end]):
                        ...
                        for on, otherNeighbor in enumerate(seg.links[1-end]):
                            add_link(newSegment, 1-end, otherNeighbor, seg.otherEndOfLinks[1-end][on], seg.CIGARs[1-end][on])
                        segments.append(newSegment)
