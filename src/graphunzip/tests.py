...
from graphunzip.input_output import load_gfa
from graphunzip.input_output import export_to_GFA
from graphunzip.input_output import load_interactionMatrix
from graphunzip.solve_ambiguities import solve_ambiguities
from graphunzip.solve_ambiguities import intensity_of_interactions
from graphunzip.solve_ambiguities import merge_adjacent_contigs
...
def blast_find_contig(contig, gfaFile, database):
    ...
    cmd = (
        "blastn -query trash_me.fa -db "
        + database
        + " -outfmt 5 -out ttrash_me.xml -task megablast"
    )
    os.system(cmd)
    ...
    for blast_record in blast_records:
        ...
        for alignment in blast_record.alignments:
            for hsp in alignment.hsps:
                if (
                    hsp.align_length == hsp.identities
                    and hsp.identities == blast_record.query_length
                ):
                    checked = True
                    print(
                        contig,
                ...
                if (
                    hsp.identities > 0.98 * hsp.align_length
                    and hsp.identities > 0.95 * blast_record.query_length
                    and not checked
                ):
                    print(
                        contig,
...
def blast_solution(solutionFile, fastaAssemblyDtb, outdirectory, chunks=1000):
    ...
    for query in queryfiles:
        ...
        blastn_cline = NcbiblastnCommandline(
            query=outdirectory + "/" + query,
            db=fastaAssemblyDtb,
            evalue=0.0000001,
            outfmt=5,
            out=outdirectory + "/blast_sol" + query.strip(".fasta") + ".xml",
            task="megablast",
        )
        out, err = blastn_cline()

        result_handle = open(
            outdirectory + "/blast_sol" + query.strip(".fasta") + ".xml"
        )
        blast_records = NCBIXML.parse(result_handle)
...
def buildFakeChromosomes(chromosomesLength=10):
    ...
    chromosomes = [
        [letters[j] + str(i) for i in range(chromosomesLength)] for j in range(4)
    ]

    duplicates = (
        0  # number of conitgs that are going to be repeated within each chromosomes :
    )
        ...
        for contig in c:
            if contig not in alreadyExported:
                f.write(
                    ...
                    + contig
                    + "\tCT"
                    + "".join(["A" for i in range(lengthOfContig - 2)])
                    + "CT"
                    + "\n"
                )
            ...
            for c2 in range(c1, len(c)):
                ...
                if con1 != "T" and con2 != "T":  # because 'T' elements are transposons
                    intensity = integrate.quad(
                        dist_law,
                        (c2 - c1 - 1) * lengthOfContigs,
                        (c2 - c1) * lengthOfContigs,
                    )[0]
...
def check_result(chromosomes, listOfSuperContigs, names):
    ...
    expectedContacts = sparse.dok_matrix(
        (len(names), len(names))
    )  # this is the matrix of all the links that are in the solution and therefore are expected in the output
    for c in chromosomes:
        for contig in range(len(c) - 1):
    ...
    for segment in listOfSuperContigs:
        for endOfSegment in range(2):
            for l, neighbor in enumerate(segment.links[endOfSegment]):
                expectedContacts[
                    names[segment.names[-endOfSegment]],
                    names[neighbor.names[-segment.otherEndOfLinks[endOfSegment][l]]],
                ] = 0
    ...
    for i in range(len(names)):
        for j in range(len(names)):
            if expectedContacts[i, j] == 1:
                print(
                    "In the actual chromosomes, there exist a link between ",
                    i,
                    " and ",
                    j,
                    " that is not found in the output",
                )
                print(names)
...
def stats_on_solve_ambiguities(n=100, lengthOfChromosomes=10, steps=10):
    ...
    for i in range(n):
        ...
        exportFakeToGFA(
            chromosomes, "tests/stats/test" + str(i) + ".gfa", lengthOfContig
        )
        bf.export_to_csv(chromosomes, "tests/stats/test" + str(i) + ".chro")

        listOfSegments, names = load_gfa("tests/stats/test" + str(i) + ".gfa")

        interactionMatrix = constructFakeInteractionMatrix(
            chromosomes, names, lengthOfContig
        )

        listOfSegments = solve_ambiguities(
            listOfSegments, interactionMatrix, 0.2, 0.45, 5
        )  # rejectedThreshold<AcceptedThreshold

        record.append(check_result(chromosomes, listOfSegments, names))

        bf.export_to_GFA(
            listOfSegments, exportFile="tests/stats/test" + str(i) + "F.gfa"
        )
...
interactionMatrix = load_interactionMatrix(
    "data_A_vaga_HiFi/Flye/interactionMatrix.pickle", segments, names
)
