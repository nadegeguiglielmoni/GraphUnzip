...
def longReads_interactionsMatrix(
    gafFile, names, segments, similarity_threshold=0, whole_mapping=False
):
    ...
    for line in f:
        ...
        if ls[5].count(">") + ls[5].count("<") > 1:
            if (not "id:f" in ls[-2]) or (
                ...
                if not whole_mapping or (
                    float(ls[2]) == 0 and float(ls[1]) == float(ls[3])
                ):
                    ...
                    for c1 in range(len(contigs) - 1):
                        for c2 in range(c1 + 1, len(contigs)):
                            if (
                                contigs[c1] not in noNames
                                and contigs[c2] not in noNames
                            ):
                                ...
                                if contigs[c1] not in namesPlus:
                                    ...
                                    if not found:
                                        noNames.add(
                                            contigs[c1]
                                        )  # if you do not find it once, the cause is lost
                                ...
                                if (
                                    contigs[c1] in namesPlus
                                    and contigs[c2] in namesPlus
                                ):
                                    if namesPlus[contigs[c1]] != namesPlus[contigs[c2]]:
                                        interactionMatrix[
                                            namesPlus[contigs[c1]],
                                            namesPlus[contigs[c2]],
                                        ] += 1
                                        if c2 == c1 + 1:
                                            if (
                                                contigsPlus[contigs[c1]] == contigs[c1]
                                                and contigsPlus[contigs[c2]]
                                                == contigs[c2]
                                            ):
                                                allLinks.add(
                                                    (
                                                        contigsPlus[contigs[c1]],
                                                        orientations[c1] == ">",
                                                        contigsPlus[contigs[c2]],
                                                        orientations[c2] == "<",
                                                    )
                                                )
                                                ...
                                                contigsPlus[contigs[c2]] == contigs[c2]
                                            ):
                                                allLinks.add(
                                                    (
                                                        contigsPlus[contigs[c1]],
                                        ...
                                        repeats[namesPlus[contigs[c1]]] = max(
                                            contigs.count(contigs[c1]),
                                            repeats[namesPlus[contigs[c1]]],
                                        )
...
def how_far_away_are_those_contigs(contig1, contig2, links, infContigs):
    ...
    while pastLengthOfconnexion1 < len(connectedToContig1):
        ...
        for cc, connectedContig in enumerate(
            connectedToContig1
        ):  # that can be optimized if we need speed, since we go through the first contigs many times
            if cc < 2 or cc % 2 == 1:  # that's to move only one way in the contig graph
                for newContig in links[connectedContig]:
                    newdist = (
                        distanceToContig1[cc] + infContigs[int(connectedContig / 2)][1]
                    )
    ...
    if contig2 * 2 in connectedToContig1:
        ...
        return (
            distanceToContig1[indexContig2] - infContigs[contig1][1],
            pathToContig1[indexContig2][1:],
        )  # because the length of contig 1 is always taken into account in the path length while it shouldn't
    else:
        print(connectedToContig1)
...
def crush_small_contigs(segments, interactionMatrix):
    ...
    for segment in segments:
        if (
            segment.length < 5000 and segment in segment.links[0]
        ):  # this characterize a small repeated sequence
            if (
                segment.HiCcoverage == 0
            ):  # this segment is blind, can't do anything but crush it
                segment.ID = (
                    -segment.ID
                )  # segments marked with negative IDs should be deleted
...
def solve_small_loops(listOfSegments, names, repeats, lr_links, check_links):
    for se in range(len(listOfSegments)):
        ...
        if segment in segment.links[0]:  # this is a small loop of length 0
            if (
                segment.links[0].count(segment) == 1
            ):  # this is a o-loop, let's flatten it
                if repeats != []:
                    replications = 0
        ...
        for n, neighbor in enumerate(
            segment.links[0]
        ):  # trying to detect o-loops of length 1
            endOfLink = segment.otherEndOfLinks[0][n]
            index = s.find_this_link(
                neighbor,
                1 - endOfLink,
                segment.links[1],
                segment.otherEndOfLinks[1],
                warning=False,
            ...
            if index != -1:  # then this is a o-loop of length 1
                if lr_links != []:
                    ...
                    if (
                        not (cA0, oA0, cB0, oB0) in lr_links
                        and not (cB0, oB0, cA0, oA0) in lr_links
                    ):
                        if (segment, 0, neighbor, endOfLink) not in toRemove and (
                            neighbor,
                            endOfLink,
                            segment,
                            0,
                    ...
                    if (
                        not (cA1, oA1, cB1, oB1) in lr_links
                        and not (cB1, oB1, cA1, oA1) in lr_links
                    ):
                        if (segment, 1, neighbor, endOfLink2) not in toRemove and (
                            neighbor,
                            endOfLink2,
                            segment,
                            1,
...
def solve_l_loops(
    segments, lr_links
):  # l-loops occur when one end of a contig is in contact with both end of another contig or when a contig is linked to itself at one end
    for segment in segments:
        for endOfSegment in range(2):
            toRemove = []
            ...
            for n in range(len(segment.links[endOfSegment]) - 1):
                if (
                    ...
                    and segment.otherEndOfLinks[endOfSegment][n]
                    == segment.otherEndOfLinks[endOfSegment][n + 1]
                ):  # the two links going toward the same contig are next to each other because links are sorted
                    ...
                    if lr_links != []:
                        ...
                        if (
                            ...
                            cB0 = neighbor.names[
                                -segment.otherEndOfLinks[endOfSegment][n]
                            ]
                            oB0 = (
                            ...
                            if (
                                not (cA0, oA0, cB0, oB0) in lr_links
                                and not (cB0, not oB0, cA0, not oA0) in lr_links
                            ):
                                toRemove += [
                                    (segment, endOfSegment, neighbor, int(oB0))
                                ]
                            ...
                            if (
                                not (cA1, oA1, cB1, oB1) in lr_links
                                and not (cB1, not oB1, cA1, not oA1) in lr_links
                            ):
                                toRemove += [
                                    (segment, endOfSegment, neighbor, int(oB1))
                                ]

                    if (
                        len(toRemove) == lenToRemove
                    ):  # means that no links could be taken out
        ...
        for endOfSegment in range(2):
            ...
            if len(l) > 1 and not segment.freezed[endOfSegment]:
                ...
                if startMerging:  # if nothing is locked for now
                    ...
                    if (
                        multiplicities[names[segment.names[-endOfSegment]]]
                        >= len(segment.links[endOfSegment])
                        * copiesnumber[segment.names[-endOfSegment]]
                    ):
                        duplicate_around_this_end_of_contig(
                            segment, endOfSegment, listOfSegments, copiesnumber
                        )
                    ...
                    if verbose:
                        print(
                            ...
                            [i.names for i in listOfSegments],
                        )
...
def get_rid_of_bad_links(
    ...
    thresholdAccepted,
    debugDir="",
    neighborsOfNeighbors=True,
    verbose=False,
):
    HiCmatrix = (
        interactionMatrix.count_nonzero() > 0
    )  # a boolean value to tell if there is need to use the Hi-C interaction matrix
    ...
    for segment in listOfSegments:
        ...
        for endOfSegment in range(2):
            if (
                len(segment.links[endOfSegment]) >= 2
            ):  # then it means that there is a choice to be made at one end of the segment. Let's see how HiC contacts confirm those links
                ...
                for n1 in range(len(segment.links[endOfSegment]) - 1):
                    ...
                    while n2 < len(segment.links[endOfSegment]):
                        ...
                        if tagInteractionMatrix.count_nonzero() > 0 and (
                            linksStrength == [-1]
                            or (
                                all([i > 1 for i in linksStrength])
                                or all([i <= 1 for i in linksStrength])
                            )
                        ):
                            linksStrength = intensity_of_interactions(
                                segment,
                                endOfSegment,
                                [n1, n2],
                                listOfSegments,
                        ...
                        if debugDir != "":
                            ...
                            f.write(
                                ...
                                + " between "
                                + "_".join(segment.links[endOfSegment][n1].names)
                                + " and "
                                + "_".join(segment.links[endOfSegment][n2].names)
                                + " with these values : "
                                + str(linksStrength)
                                + "\n"
                            )
                        ...
                        if linksStrength == [
                            -1
                        ]:  # means that the configuration does not enable the algorithm to compare the two interactions
                            ...
                            if linksStrength[0] > linksStrength[1]:
                                if (
                                    linksStrength[1]
                                    <= linksStrength[0] * thresholdRejected
                                ) or (
                                    linksStrength[1] == 1 and linksStrength[0] > 2
                                ):  # then it means that probably link does not exist
                                    # however, we'd like not to create any dead end, hence a little precaution:
                                    if (
                                        len(
                                            segment.links[endOfSegment][n2].links[
                                                segment.otherEndOfLinks[endOfSegment][
                                                    n2
                                                ]
                                            ]
                                        )
                                    ...
                                    linksStrength[1]
                                    < linksStrength[0] * thresholdAccepted
                                ):  # then it's not clear, the link is freezed
                                    segment.freezeNode(endOfSegment)

                            else:
                                if linksStrength[0] < linksStrength[
                                    1
                                ] * thresholdRejected or (
                                    linksStrength[0] == 1 and linksStrength[1] > 2
                                ):  # then decide that the link does not exist
                                    # however, we'd like not to create any dead end, hence a little precaution:
                                    if (
                                        len(
                                            segment.links[endOfSegment][n1].links[
                                                segment.otherEndOfLinks[endOfSegment][
                                                    n1
                                                ]
                                            ]
                                    ...
                                    linksStrength[0]
                                    < linksStrength[1] * thresholdAccepted
                                ):  # then it's not clear, the link is freezed
                                    segment.freezeNode(endOfSegment)
                ...
                for n in toRemove:
                    segment._links[endOfSegment][n].remove_end_of_link(
                        segment._otherEndOfLinks[endOfSegment][n], segment, endOfSegment
                    )
                    segment.remove_end_of_link(
                        endOfSegment,
                        segment._links[endOfSegment][n],
                        segment._otherEndOfLinks[endOfSegment][n],
                    )
            ...
            if (
                ...
                for n1 in range(len(segment.links[endOfSegment]) - 1):
                    ...
                    while n2 < len(segment.links[endOfSegment]):
                        ...
                        (
                            absoluteLinksStrength,
                            linksStrength,
                            neighborsOfNeighborsUsed,
                        ) = intensity_of_interactions(
                            segment,
                            [
                                segment.links[endOfSegment][n1],
                                segment.links[endOfSegment][n2],
                            ],
                            [
                                segment.otherEndOfLinks[endOfSegment][n1],
...
def solve_ambiguities(
    ...
    for i in range(steps):
        get_rid_of_bad_links(
            ...
            stringenceAccept,
            debugDir=debugDir,
            neighborsOfNeighbors=useNeighborOfNeighbor,
            verbose=verbose,
        )
        ...
        listOfSegments, copiesNumber = merge_contigs(
            listOfSegments, copiesNumber, multiplicities, names, verbose=verbose
        )
    ...
    if interactionMatrix.count_nonzero() > 0:
        listOfSegments = break_up_chimeras(
            listOfSegments, names, interactionMatrix, 100000
        )

    return (
        listOfSegments,
        copiesNumber,
    )  # return copiesNumber in case you want to run solve_ambiguities several times in a row
